...
exports.handler = async function(event, context) {
  ...
  try {
    if (event.httpMethod === 'POST') {
      ...
      return user ? { statusCode: 200, headers, body: JSON.stringify({ success: true, user }) } : { statusCode: 401, headers, body: JSON.stringify({ error: 'Invalid credentials' }) };
    }
